...
NodeList(RootNode).h = (abs(NodeList(TargetNode).x-NodeList(RootNode).x)+abs(NodeList(TargetNode).y-NodeList(RootNode).y));
    ...
    for i = 1:length(NodeList(nbest).Neigh)
        if(not(ismember(NodeList(nbest).Neigh(i),Closed)))
            if(not(ismember(NodeList(nbest).Neigh(i),Open)))
                ...
                NodeList(NodeList(nbest).Neigh(i)).h = (abs(NodeList(TargetNode).x-NodeList(NodeList(nbest).Neigh(i)).x)+abs(NodeList(TargetNode).y-NodeList(NodeList(nbest).Neigh(i)).y));
            else
